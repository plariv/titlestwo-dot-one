...
$layout->blocks["top"][] = "details";$page_layouts["properties_edit"] = $layout;
...
$templatefile = "properties_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["idproperties"]=urldecode(postvalue("editid1"));
$savedKeys["idproperties"]=urldecode(postvalue("editid1"));
...
//	processing Platte Number - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$value = postvalue("value_Platte_Number_".$id);
		$type = postvalue("type_Platte_Number_".$id);
		if(FieldSubmitted("Platte Number_".$id))
		{
				$value = prepare_for_db("Platte Number",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "Platte Number"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Platte Number"] = $value;
		
			}
	
		}
//	processing Platte Number - end
//	processing Plot Number - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$value = postvalue("value_Plot_Number_".$id);
		$type = postvalue("type_Plot_Number_".$id);
		if(FieldSubmitted("Plot Number_".$id))
		{
				$value = prepare_for_db("Plot Number",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "Plot Number"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Plot Number"] = $value;
		
			}
	
		}
//	processing Plot Number - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idproperties"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: properties_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_properties->Copy();
...
if($readevalues)
{
	...
	$data["Platte Number"] = $evalues["Platte Number"];
	$data["Plot Number"] = $evalues["Plot Number"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Platte Number"))
		$xt->assign("Platte_Number_fieldblock",true);
	else
		$xt->assign("Platte_Number_tabfieldblock",true);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "properties";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='properties_view.php?".$viewlink."'\"");
