...
$layout->blocks["top"][] = "details";$page_layouts["parties_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "parties_inline_add.htm";
else
	$templatefile = "parties_add.htm";
...
//	processing Contact Name - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["Contact Name"]=$value;
		}
		}
//	processibng Contact Name - end
//	processing Company - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["Company"]=$value;
		}
		}
//	processibng Company - end
...
//	processing Address line 2 - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["Address line 2"]=$value;
		}
		}
//	processibng Address line 2 - end
//	processing ZIP Code - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["ZIP Code"]=$value;
		}
		}
//	processibng ZIP Code - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'parties_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'parties_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: parties_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["idparties"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["idparties"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["Contact Name"]=@$avalues["Contact Name"];
	$defvalues["Company"]=@$avalues["Company"];
	$defvalues["Address line 1"]=@$avalues["Address line 1"];
	$defvalues["Address line 2"]=@$avalues["Address line 2"];
	$defvalues["City"]=@$avalues["City"];
	$defvalues["State"]=@$avalues["State"];
	$defvalues["ZIP Code"]=@$avalues["ZIP Code"];
}
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["buyers"]["masterkey1"] = $data["idparties"];	
	$showDetailKeys["sellers"]["masterkey1"] = $data["idparties"];	

	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["idparties"]));
...
//	Contact Name - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Contact Name", ""),"field=Contact+Name".$keylink,"",MODE_LIST);
	$showValues["Contact Name"] = $value;
	$showFields[] = "Contact Name";
		$showRawValues["Contact Name"] = substr($data["Contact Name"],0,100);
	}	
//	Company - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Company", ""),"field=Company".$keylink,"",MODE_LIST);
...
//	Address line 1 - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Address line 1", ""),"field=Address+line+1".$keylink,"",MODE_LIST);
	$showValues["Address line 1"] = $value;
	$showFields[] = "Address line 1";
		$showRawValues["Address line 1"] = substr($data["Address line 1"],0,100);
	}	
//	Address line 2 - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Address line 2", ""),"field=Address+line+2".$keylink,"",MODE_LIST);
	$showValues["Address line 2"] = $value;
	$showFields[] = "Address line 2";
		$showRawValues["Address line 2"] = substr($data["Address line 2"],0,100);
	}	
//	City - 
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	ZIP Code - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"ZIP Code", ""),"field=ZIP+Code".$keylink,"",MODE_LIST);
	$showValues["ZIP Code"] = $value;
	$showFields[] = "ZIP Code";
		$showRawValues["ZIP Code"] = substr($data["ZIP Code"],0,100);
	}	
