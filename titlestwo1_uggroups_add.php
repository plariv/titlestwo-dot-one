...
$layout->blocks["top"][] = "details";$page_layouts["titlestwo1_uggroups_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "titlestwo1_uggroups_inline_add.htm";
else
	$templatefile = "titlestwo1_uggroups_add.htm";
...
//	processing Label - start
	...
	if($inlineAddOption)
	{
		$value = postvalue("value_Label_".$id);
		$type=postvalue("type_Label_".$id);
		if (FieldSubmitted("Label_".$id))
		{
				$value=prepare_for_db("Label",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Label"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Label"]=$value;
		}
		}
//	processibng Label - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'titlestwo1_uggroups_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'titlestwo1_uggroups_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: titlestwo1_uggroups_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["GroupID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["GroupID"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Label"))
		$xt->assign("Label_fieldblock",true);
	else
		$xt->assign("Label_tabfieldblock",true);
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["GroupID"]));
...
//	GroupID - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"GroupID", ""),"field=GroupID".$keylink,"",MODE_LIST);
...
//	Label - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Label", ""),"field=Label".$keylink,"",MODE_LIST);
	$showValues["Label"] = $value;
	$showFields[] = "Label";
		$showRawValues["Label"] = substr($data["Label"],0,100);
	}	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "titlestwo1_uggroups";
		$options['firstTime'] = 1;
