...
$layout->blocks["top"][] = "details";$page_layouts["lastfilenumbers_edit"] = $layout;
...
$templatefile = "lastfilenumbers_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["fileyear"]=urldecode(postvalue("editid1"));
$savedKeys["fileyear"]=urldecode(postvalue("editid1"));
...
//	processing fileyear - end
//	processing lastfile - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "lastfile"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["lastfile"] = $value;
		
			}
	
		}
//	processing lastfile - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["fileyear"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lastfilenumbers_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_lastfilenumbers->Copy();
...
if(!$data)
{
	...
	{
		header("Location: lastfilenumbers_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["fileyear"] = $evalues["fileyear"];
	$data["lastfile"] = $evalues["lastfile"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"fileyear", "")));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lastfilenumbers_view.php?".$viewlink."'\"");
