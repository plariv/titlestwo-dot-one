...
$layout->blocks["top"][] = "details";$page_layouts["brokers_edit"] = $layout;
...
$skeys = "";
$savedKeys = array();
$keys["idbrokers"]=urldecode(postvalue("editid1"));
$savedKeys["idbrokers"]=urldecode(postvalue("editid1"));
...
//	processing agents_idagents - begin
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "agents_idagents"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["agents_idagents"] = $value;
		
			}
	
		}
//	processing agents_idagents - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idbrokers"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: brokers_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: brokers_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["agents_idagents"] = $evalues["agents_idagents"];
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='brokers_view.php?".$viewlink."'\"");
