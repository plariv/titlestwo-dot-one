...
$layout->blocks["top"][] = "details";$page_layouts["ttlaction_notes_edit"] = $layout;
...
$templatefile = "ttlaction_notes_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["idnotes"]=urldecode(postvalue("editid1"));
$savedKeys["idnotes"]=urldecode(postvalue("editid1"));
...
//	processing ttlactions_idttlactions - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "ttlactions_idttlactions"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["ttlactions_idttlactions"] = $value;
		
			}
	
		}
//	processing ttlactions_idttlactions - end
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "note_content"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["note_content"] = $value;
		
			}
	
		}
//	processing note_content - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idnotes"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ttlaction_notes_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ttlaction_notes->Copy();
...
if(!$data)
{
	...
	{
		header("Location: ttlaction_notes_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["ttlactions_idttlactions"] = $evalues["ttlactions_idttlactions"];
	$data["created"] = $evalues["created"];
	$data["note_content"] = $evalues["note_content"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idnotes", "")));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='ttlaction_notes_view.php?".$viewlink."'\"");
