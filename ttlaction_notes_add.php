...
$layout->blocks["top"][] = "details";$page_layouts["ttlaction_notes_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "ttlaction_notes_inline_add.htm";
else
	$templatefile = "ttlaction_notes_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing ttlactions_idttlactions - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_ttlactions_idttlactions_".$id);
		$type=postvalue("type_ttlactions_idttlactions_".$id);
		if (FieldSubmitted("ttlactions_idttlactions_".$id))
		{
				$value=prepare_for_db("ttlactions_idttlactions",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "ttlactions_idttlactions"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["ttlactions_idttlactions"]=$value;
		}
		}
//	processibng ttlactions_idttlactions - end
//	processing created - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_created_".$id);
		$type=postvalue("type_created_".$id);
		if (FieldSubmitted("created_".$id))
		{
		...
		if(!($value===false))
		{
						...
						if(0 && "created"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["created"]=$value;
		}
		}
//	processibng created - end
//	processing note_content - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_note_content_".$id);
		$type=postvalue("type_note_content_".$id);
		if (FieldSubmitted("note_content_".$id))
		{
				$value=prepare_for_db("note_content",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "note_content"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["note_content"]=$value;
		}
		}
//	processibng note_content - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="ttl_actions")
	{
		...
		if($avalues["ttlactions_idttlactions"]=="")
			$avalues["ttlactions_idttlactions"]=prepare_for_db("ttlactions_idttlactions",$_SESSION[$sessionPrefix."_masterkey1"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'ttlaction_notes_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'ttlaction_notes_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ttlaction_notes_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["idnotes"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["idnotes"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="ttl_actions")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["ttlactions_idttlactions"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["ttlactions_idttlactions"]=@$avalues["ttlactions_idttlactions"];
	$defvalues["created"]=@$avalues["created"];
	$defvalues["note_content"]=@$avalues["note_content"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("ttlactions_idttlactions"))
		$xt->assign("ttlactions_idttlactions_fieldblock",true);
	else
		$xt->assign("ttlactions_idttlactions_tabfieldblock",true);
	$xt->assign("ttlactions_idttlactions_label",true);
	if(isEnableSection508())
		$xt->assign_section("ttlactions_idttlactions_label","<label for=\"".GetInputElementId("ttlactions_idttlactions", $id)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("created_label","<label for=\"".GetInputElementId("created", $id)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("note_content"))
		$xt->assign("note_content_fieldblock",true);
	else
		$xt->assign("note_content_tabfieldblock",true);
	$xt->assign("note_content_label",true);
	if(isEnableSection508())
		$xt->assign_section("note_content_label","<label for=\"".GetInputElementId("note_content", $id)."\">","</label>");
...
//	idnotes - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"idnotes", ""),"field=idnotes".$keylink,"",MODE_LIST);
	$showValues["idnotes"] = $value;
	$showFields[] = "idnotes";
		$showRawValues["idnotes"] = substr($data["idnotes"],0,100);
	}	
//	ttlactions_idttlactions - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"ttlactions_idttlactions", ""),"field=ttlactions%5Fidttlactions".$keylink,"",MODE_LIST);
	$showValues["ttlactions_idttlactions"] = $value;
	$showFields[] = "ttlactions_idttlactions";
		$showRawValues["ttlactions_idttlactions"] = substr($data["ttlactions_idttlactions"],0,100);
	}	
//	created - Short Date
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"created", "Short Date"),"field=created".$keylink,"",MODE_LIST);
...
//	note_content - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"note_content", ""),"field=note%5Fcontent".$keylink,"",MODE_LIST);
	$showValues["note_content"] = $value;
	$showFields[] = "note_content";
		$showRawValues["note_content"] = substr($data["note_content"],0,100);
	}	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "ttlaction_notes";
		$options['firstTime'] = 1;
