...
$layout->blocks["top"][] = "details";$page_layouts["ttl_recordings_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "ttl_recordings_inline_add.htm";
else
	$templatefile = "ttl_recordings_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing Mortgage - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_Mortgage_".$id);
		$type=postvalue("type_Mortgage_".$id);
		if (FieldSubmitted("Mortgage_".$id))
		{
				$value=prepare_for_db("Mortgage",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Mortgage"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Mortgage"]=$value;
		}
		}
//	processibng Mortgage - end
//	processing Discharge recorded (Y/N) - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_Discharge_recorded__Y_N__".$id);
		$type=postvalue("type_Discharge_recorded__Y_N__".$id);
		if (FieldSubmitted("Discharge recorded (Y/N)_".$id))
		{
				$value=prepare_for_db("Discharge recorded (Y/N)",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Discharge recorded (Y/N)"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Discharge recorded (Y/N)"]=$value;
		}
		}
//	processibng Discharge recorded (Y/N) - end
//	processing Recording Date - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_Recording_Date_".$id);
		$type=postvalue("type_Recording_Date_".$id);
		if (FieldSubmitted("Recording Date_".$id))
		{
				$value=prepare_for_db("Recording Date",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Recording Date"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Recording Date"]=$value;
		}
		}
//	processibng Recording Date - end
//	processing Book - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_Book_".$id);
		$type=postvalue("type_Book_".$id);
		if (FieldSubmitted("Book_".$id))
		{
				$value=prepare_for_db("Book",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Book"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Book"]=$value;
		}
		}
//	processibng Book - end
//	processing Page - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_Page_".$id);
		$type=postvalue("type_Page_".$id);
		if (FieldSubmitted("Page_".$id))
		{
				$value=prepare_for_db("Page",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Page"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Page"]=$value;
		}
		}
//	processibng Page - end
//	processing Account Number - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_Account_Number_".$id);
		$type=postvalue("type_Account_Number_".$id);
		if (FieldSubmitted("Account Number_".$id))
		{
				$value=prepare_for_db("Account Number",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Account Number"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Account Number"]=$value;
		}
		}
//	processibng Account Number - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="ttl_actions")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["ttl_actions_idttl_actions"]=="")
			$avalues["ttl_actions_idttl_actions"]=prepare_for_db("ttl_actions_idttl_actions",$_SESSION[$sessionPrefix."_masterkey1"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'ttl_recordings_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'ttl_recordings_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ttl_recordings_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["idttl_recordings"]=postvalue("copyid1");
		$copykeys["ttl_actions_idttl_actions"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["idttl_recordings"]=postvalue("editid1");
		$copykeys["ttl_actions_idttl_actions"]=postvalue("editid2");
	}
	...
	if(!$defvalues)
		$defvalues=array();
//	clear key fields
	$defvalues["idttl_recordings"]="";
	$defvalues["ttl_actions_idttl_actions"]="";
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="ttl_actions")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["ttl_actions_idttl_actions"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["Mortgage"]=@$avalues["Mortgage"];
	$defvalues["Discharge recorded (Y/N)"]=@$avalues["Discharge recorded (Y/N)"];
	$defvalues["Recording Date"]=@$avalues["Recording Date"];
	$defvalues["Book"]=@$avalues["Book"];
	$defvalues["Page"]=@$avalues["Page"];
	$defvalues["Account Number"]=@$avalues["Account Number"];
}
...
//	Mortgage - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Mortgage", ""),"field=Mortgage".$keylink,"",MODE_LIST);
	$showValues["Mortgage"] = $value;
	$showFields[] = "Mortgage";
		$showRawValues["Mortgage"] = substr($data["Mortgage"],0,100);
	}	
//	Discharge recorded (Y/N) - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Discharge recorded (Y/N)", ""),"field=Discharge+recorded+%28Y%2FN%29".$keylink,"",MODE_LIST);
	$showValues["Discharge recorded (Y/N)"] = $value;
	$showFields[] = "Discharge recorded (Y/N)";
		$showRawValues["Discharge recorded (Y/N)"] = substr($data["Discharge recorded (Y/N)"],0,100);
	}	
//	Recording Date - Short Date
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Recording Date", "Short Date"),"field=Recording+Date".$keylink,"",MODE_LIST);
	$showValues["Recording Date"] = $value;
	$showFields[] = "Recording Date";
		$showRawValues["Recording Date"] = substr($data["Recording Date"],0,100);
	}	
//	Book - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Book", ""),"field=Book".$keylink,"",MODE_LIST);
	$showValues["Book"] = $value;
	$showFields[] = "Book";
		$showRawValues["Book"] = substr($data["Book"],0,100);
	}	
//	Page - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Page", ""),"field=Page".$keylink,"",MODE_LIST);
	$showValues["Page"] = $value;
	$showFields[] = "Page";
		$showRawValues["Page"] = substr($data["Page"],0,100);
	}	
//	Account Number - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Account Number", ""),"field=Account+Number".$keylink,"",MODE_LIST);
	$showValues["Account Number"] = $value;
	$showFields[] = "Account Number";
		$showRawValues["Account Number"] = substr($data["Account Number"],0,100);
	}	
