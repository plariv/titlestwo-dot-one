...
include("include/buyersactions_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["buyersactions_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "buyersactions_search.htm";
...
$params['shortTableName'] = 'buyersactions';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if(GetLookupTable("idbuyers", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("idbuyers", $strTableName)] = GetTableURL(GetLookupTable("idbuyers", $strTableName));
	
	$pageObject->fillFieldToolTips("idbuyers");	
		
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("idbuyers");
	...
	{
		$firstFieldParams['fName'] = "idbuyers";
	...
	if(isEnableSection508())
		$xt->assign_section("idbuyers_label","<label for=\"".GetInputElementId("idbuyers", $id)."\">","</label>");
	else 
		$xt->assign("idbuyers_label", true);
	
	$xt->assign("idbuyers_fieldblock", true);		
	$xt->assignbyref("idbuyers_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("idbuyers_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("idbuyers_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_idbuyers", $ctrlBlockArr['searchtype']);	
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("idbuyers");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl) 
	{				
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"idbuyers", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{	
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"idbuyers", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
