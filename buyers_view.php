...
$layout->blocks["top"][] = "details";$page_layouts["buyers_view"] = $layout;
...
//	show one record only
	$keys=array();
	$strWhereClause="";
	$keys["idbuyers"]=postvalue("editid1");
...
while($data)
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["idbuyers"]));
...
/*$jsKeysObj = 'window.recKeysObj = {';
	$jsKeysObj .= "'".jsreplace("idbuyers")."': '".(jsreplace(@$data["idbuyers"]))."', ";
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='buyers_edit.php?".$editlink."'\"");
