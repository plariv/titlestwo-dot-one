...
$layout->blocks["top"][] = "details";$page_layouts["agents_edit"] = $layout;
...
$skeys = "";
$savedKeys = array();
$keys["idagents"]=urldecode(postvalue("editid1"));
$savedKeys["idagents"]=urldecode(postvalue("editid1"));
...
//	processing name - end
//	processing salutation - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$value = postvalue("value_salutation_".$id);
		$type = postvalue("type_salutation_".$id);
		if(FieldSubmitted("salutation_".$id))
		{
				$value = prepare_for_db("salutation",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "salutation"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["salutation"] = $value;
		
			}
	
		}
//	processing salutation - end
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "co_name"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["co_name"] = $value;
		
			}
	
		}
//	processing co_name - end
//	processing addr1 - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
...
//	processing addr1 - end
//	processing addr2 - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idagents"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: agents_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: agents_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["salutation"] = $evalues["salutation"];
	$data["phone"] = $evalues["phone"];
	$data["contact"] = $evalues["contact"];
	$data["co_name"] = $evalues["co_name"];
	$data["addr1"] = $evalues["addr1"];
	$data["addr2"] = $evalues["addr2"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("salutation"))
		$xt->assign("salutation_fieldblock",true);
	else
		$xt->assign("salutation_tabfieldblock",true);
	...
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idagents", "")));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='agents_view.php?".$viewlink."'\"");
