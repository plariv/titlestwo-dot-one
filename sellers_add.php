...
add_nocache_headers();
include("include/sellers_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["sellers_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "sellers_inline_add.htm";
else
	$templatefile = "sellers_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing parties_idparties - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_parties_idparties_".$id);
		$type=postvalue("type_parties_idparties_".$id);
		if (FieldSubmitted("parties_idparties_".$id))
		{
				$value=prepare_for_db("parties_idparties",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "parties_idparties"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["parties_idparties"]=$value;
		}
		}
//	processibng parties_idparties - end
...
//	insert masterkey value if exists and if not specified
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="parties")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["parties_idparties"]=="")
			$avalues["parties_idparties"]=prepare_for_db("parties_idparties",$_SESSION[$sessionPrefix."_masterkey1"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'sellers_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'sellers_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: sellers_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["idsellers"]=postvalue("copyid1");
		$copykeys["ttl_actions_idttl_actions"]=postvalue("copyid2");
		$copykeys["parties_idparties"]=postvalue("copyid3");
	}
	else
	{
		$copykeys["idsellers"]=postvalue("editid1");
		$copykeys["ttl_actions_idttl_actions"]=postvalue("editid2");
		$copykeys["parties_idparties"]=postvalue("editid3");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="parties")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["parties_idparties"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["parties_idparties"]=@$avalues["parties_idparties"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("parties_idparties"))
		$xt->assign("parties_idparties_fieldblock",true);
	else
		$xt->assign("parties_idparties_tabfieldblock",true);
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["idsellers"]));
	$keylink.="&key2=".htmlspecialchars(rawurlencode(@$data["ttl_actions_idttl_actions"]));
	$keylink.="&key3=".htmlspecialchars(rawurlencode(@$data["parties_idparties"]));
...
//	parties_idparties - 
	...
	{	
		$value="";
				$value = DisplayLookupWizard("parties_idparties",$data["parties_idparties"],$data,$keylink,MODE_LIST);
	$showValues["parties_idparties"] = $value;
	$showFields[] = "parties_idparties";
		$showRawValues["parties_idparties"] = substr($data["parties_idparties"],0,100);
	}	
//	name - 
	$display = false;
...
//	phone - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"phone", ""),"field=phone".$keylink,"",MODE_LIST);
