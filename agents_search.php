...
$layout->blocks["top"][] = "search";$page_layouts["agents_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "agents_search.htm";
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if(GetLookupTable("salutation", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("salutation", $strTableName)] = GetTableURL(GetLookupTable("salutation", $strTableName));
	
	$pageObject->fillFieldToolTips("salutation");	
		
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("salutation");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "salutation";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("salutation_label","<label for=\"".GetInputElementId("salutation", $id)."\">","</label>");
	else 
		$xt->assign("salutation_label", true);
	
	$xt->assign("salutation_fieldblock", true);		
	$xt->assignbyref("salutation_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("salutation_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("salutation_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_salutation", $ctrlBlockArr['searchtype']);	
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("salutation");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl) 
	{				
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"salutation", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{	
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"salutation", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if(GetLookupTable("idagents", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("idagents", $strTableName)] = GetTableURL(GetLookupTable("idagents", $strTableName));
	
	$pageObject->fillFieldToolTips("idagents");	
		
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("idagents");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "idagents";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("idagents_label","<label for=\"".GetInputElementId("idagents", $id)."\">","</label>");
	else 
		$xt->assign("idagents_label", true);
	
	$xt->assign("idagents_fieldblock", true);		
	$xt->assignbyref("idagents_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("idagents_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("idagents_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_idagents", $ctrlBlockArr['searchtype']);	
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("idagents");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl) 
	{				
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"idagents", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{	
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"idagents", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
