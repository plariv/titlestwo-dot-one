...
$layout->blocks["top"][] = "export";$page_layouts["titlestwo1_uggroups_export"] = $layout;
...
$xt->display("titlestwo1_uggroups_export.htm");

function ExportToExcel_old()
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=titlestwo1_uggroups.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=titlestwo1_uggroups.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=titlestwo1_uggroups.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$values = array();
			$values["GroupID"] = GetData($row,"GroupID","");
			$values["Label"] = GetData($row,"Label","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=titlestwo1_uggroups.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			$format = "";
			$values["GroupID"] = GetData($row,"GroupID",$format);
			$format = "";
			$values["Label"] = GetData($row,"Label",$format);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Label"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Label").'</td>';	
	}
	else
	{
		echo "<td>"."Group ID"."</td>";
	...
	echo "</tr>";
			$totals = array();
		$totals["GroupID"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"GroupID", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["Label"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"Label", 'totalsType'=>'', 'viewFormat'=>"");
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["GroupID"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Label"]);
					else
						echo htmlspecialchars($values["Label"]);
			echo '</td>';
			echo "</tr>";
		}
