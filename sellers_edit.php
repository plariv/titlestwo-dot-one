...
$layout->blocks["top"][] = "details";$page_layouts["sellers_edit"] = $layout;
...
$skeys = "";
$savedKeys = array();
$keys["idsellers"]=urldecode(postvalue("editid1"));
$savedKeys["idsellers"]=urldecode(postvalue("editid1"));
$skeys.=rawurlencode(postvalue("editid1"))."&";
$keys["ttl_actions_idttl_actions"]=urldecode(postvalue("editid2"));
$savedKeys["ttl_actions_idttl_actions"]=urldecode(postvalue("editid2"));
$skeys.=rawurlencode(postvalue("editid2"))."&";
$keys["parties_idparties"]=urldecode(postvalue("editid3"));
$savedKeys["parties_idparties"]=urldecode(postvalue("editid3"));
...
//	processing parties_idparties - begin
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "parties_idparties"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["parties_idparties"] = $value;
		
			}
	
				//	update key value
		if($value!==false)
			$keys["parties_idparties"] = $value;
	}
//	processing parties_idparties - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idsellers"])."&";
		$keyGetQ.="editid2=".rawurldecode($keys["ttl_actions_idttl_actions"])."&";
		$keyGetQ.="editid3=".rawurldecode($keys["parties_idparties"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: sellers_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: sellers_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["parties_idparties"] = $evalues["parties_idparties"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idsellers", "")));
	$xt->assign("show_key2", htmlspecialchars(GetData($data,"ttl_actions_idttl_actions", "")));
	$xt->assign("show_key3", htmlspecialchars(GetData($data,"parties_idparties", "")));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='sellers_view.php?".$viewlink."'\"");
