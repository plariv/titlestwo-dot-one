...
add_nocache_headers();
include("include/lastfilenumbers_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lastfilenumbers_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lastfilenumbers_inline_add.htm";
else
	$templatefile = "lastfilenumbers_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing fileyear - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_fileyear_".$id);
		$type=postvalue("type_fileyear_".$id);
		if (FieldSubmitted("fileyear_".$id))
		{
				$value=prepare_for_db("fileyear",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "fileyear"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["fileyear"]=$value;
		}
		}
//	processibng fileyear - end
//	processing lastfile - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_lastfile_".$id);
		$type=postvalue("type_lastfile_".$id);
		if (FieldSubmitted("lastfile_".$id))
		{
				$value=prepare_for_db("lastfile",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "lastfile"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["lastfile"]=$value;
		}
		}
//	processibng lastfile - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'lastfilenumbers_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'lastfilenumbers_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lastfilenumbers_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["fileyear"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["fileyear"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["fileyear"]=@$avalues["fileyear"];
	$defvalues["lastfile"]=@$avalues["lastfile"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("fileyear"))
		$xt->assign("fileyear_fieldblock",true);
	else
		$xt->assign("fileyear_tabfieldblock",true);
	$xt->assign("fileyear_label",true);
	if(isEnableSection508())
		$xt->assign_section("fileyear_label","<label for=\"".GetInputElementId("fileyear", $id)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("lastfile"))
		$xt->assign("lastfile_fieldblock",true);
	else
		$xt->assign("lastfile_tabfieldblock",true);
	$xt->assign("lastfile_label",true);
	if(isEnableSection508())
		$xt->assign_section("lastfile_label","<label for=\"".GetInputElementId("lastfile", $id)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["fileyear"]));
...
//	fileyear - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"fileyear", ""),"field=fileyear".$keylink,"",MODE_LIST);
	$showValues["fileyear"] = $value;
	$showFields[] = "fileyear";
		$showRawValues["fileyear"] = substr($data["fileyear"],0,100);
	}	
//	lastfile - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"lastfile", ""),"field=lastfile".$keylink,"",MODE_LIST);
	$showValues["lastfile"] = $value;
	$showFields[] = "lastfile";
		$showRawValues["lastfile"] = substr($data["lastfile"],0,100);
	}	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lastfilenumbers";
		$options['firstTime'] = 1;
