...
$layout->blocks["top"][] = "details";$page_layouts["brokers_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "brokers_inline_add.htm";
else
	$templatefile = "brokers_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing agents_idagents - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_agents_idagents_".$id);
		$type=postvalue("type_agents_idagents_".$id);
		if (FieldSubmitted("agents_idagents_".$id))
		{
				$value=prepare_for_db("agents_idagents",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "agents_idagents"=="password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["agents_idagents"]=$value;
		}
		}
//	processibng agents_idagents - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="agents")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["agents_idagents"]=="")
			$avalues["agents_idagents"]=prepare_for_db("agents_idagents",$_SESSION[$sessionPrefix."_masterkey1"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'brokers_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'brokers_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: brokers_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["idbrokers"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["idbrokers"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="agents")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["agents_idagents"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	$defvalues["agents_idagents"]=@$avalues["agents_idagents"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("agents_idagents"))
		$xt->assign("agents_idagents_fieldblock",true);
	else
		$xt->assign("agents_idagents_tabfieldblock",true);
	$xt->assign("agents_idagents_label",true);
	if(isEnableSection508())
		$xt->assign_section("agents_idagents_label","<label for=\"".GetInputElementId("agents_idagents", $id)."\">","</label>");
...
//	agents_idagents - 
	...
	{	
		$value="";
				$value = DisplayLookupWizard("agents_idagents",$data["agents_idagents"],$data,$keylink,MODE_LIST);
	$showValues["agents_idagents"] = $value;
	$showFields[] = "agents_idagents";
		$showRawValues["agents_idagents"] = substr($data["agents_idagents"],0,100);
	}	
//	name - 
	$display = false;
