...
$layout->blocks["top"][] = "details";$page_layouts["properties_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "properties_inline_add.htm";
else
	$templatefile = "properties_add.htm";
...
if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER || $inlineadd==ADD_POPUP)
{
	...
	if($pageObject->isShowDetailTables  && !isMobile())
	{
		...
		$countDetailsIsShow = 0;
			$dpPermis = $pageObject->getPermissions("ttl_actions");
		if ($dpPermis['add']){
			$countDetailsIsShow ++;
			$mKeys["ttl_actions"] = GetMasterKeysByDetailTable("ttl_actions", $strTableName);
			$dpParams['strTableNames'][] = "ttl_actions";
			$dpParams['ids'][] = ++$ids;
		}
...
//	processing Platte Number - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["Platte Number"]=$value;
		}
		}
//	processibng Platte Number - end
//	processing Plot Number - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["Plot Number"]=$value;
		}
		}
//	processibng Plot Number - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'properties_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'properties_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: properties_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["idproperties"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["idproperties"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["Platte Number"]=@$avalues["Platte Number"];
	$defvalues["Plot Number"]=@$avalues["Plot Number"];
}
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["ttl_actions"]["masterkey1"] = $data["idproperties"];	
...
//	Platte Number - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Platte Number", ""),"field=Platte+Number".$keylink,"",MODE_LIST);
	$showValues["Platte Number"] = $value;
	$showFields[] = "Platte Number";
		$showRawValues["Platte Number"] = substr($data["Platte Number"],0,100);
	}	
//	Plot Number - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Plot Number", ""),"field=Plot+Number".$keylink,"",MODE_LIST);
	$showValues["Plot Number"] = $value;
	$showFields[] = "Plot Number";
		$showRawValues["Plot Number"] = substr($data["Plot Number"],0,100);
	}	
